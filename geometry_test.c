...
START_TEST(test_area_triangle){
	...
	double num= coord_2d_area_triangle(&a,&b,&c);
	ck_assert(num==(10.0));
}
END_TEST
...
Suite* coord_2d_suite(void)
{
    ...
    TCase* tc_area_triangle = tcase_create("coord_2d_area_triangle");
    tcase_add_test(tc_area_triangle, test_area_triangle);

    /* Add Cases to Suite */
    suite_add_tcase(s, tc_2d_eq);
    suite_add_tcase(s, tc_2d_dist);
    suite_add_tcase(s, tc_2d_midpoint);
    suite_add_tcase(s, tc_area_triangle);
